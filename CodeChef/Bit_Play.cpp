...
void sol(int tc)
{
    ...
    int res = 1;
    for(int i = 3; i<=n; i+= 2){
        if((s[i-1] == '0' and s[i-2] == '1' and s[i] == '1') || (s[i-1] == '1' and s[i-2] == '0' and s[i] == '1') ){// 101, 011
            res *= 2;
            res %= mod;
        }
        else if((s[i-2] == '1' and s[i-1] == '0' and s[i] == '0') || (s[i-2] == '0' and s[i-1] == '1' and s[i] == '0')){ // 100, 010
            res *= 1;
            res %= mod;
        }
        else if(s[i] == '0' and s[i-1] == '0' and s[i-2] == '0'){// 1
            res *= 3;
            res %= mod;
        }
        else if(s[i] == '1' and s[i-1] == '1' and s[i-2] == '1'){//1
            res *= 2;
            res %= mod;
        }else if(s[i] == '0' and s[i-1] == '1' and s[i-2] == '1'){// 1
            res *= 1;
            res %= mod;
        }else if(s[i-2] == '0' and s[i-1] == '0' and s[i] == '1'){// 1
            res *= 0;
        }
    }
    cout << res << endl;
