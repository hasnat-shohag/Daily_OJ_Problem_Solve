#include <bits/stdc++.h>
using namespace std;

#define FastIO ios_base::sync_with_stdio(0), cin.tie(0)
#define TxtIO  freopen("input.txt", "r", stdin); freopen("output.txt", "w", stdout);
#define f0(i, n) for (int i = 0; i < n; i++)
#define f1(i, n) for (int i = 1; i <= n; i++)
#define f2(i, n) for (int i = 1; i < n; i++)
#define endl "\n"
#define pb push_back
//#define mp make_pair
#define vi vector<int>
#define pi pair<int, int>
#define all(x) x.begin(), x.end()
#define sqrt sqrtl
#define ff first
#define ss second
#define int long long
#define INF 1000000000
#define mod 1000000007
//_________________Function_______________
int nPr(int n, int r){ if (r > n / 2) r = n - r; int result = 1; for (int i = 1; i <= r; ++i) { result *= n - i + 1;} return result; }
int nCr(int n, int r){ if (r > n / 2) r = n - r; int result = 1; for (int i = 1; i <= r; ++i) { result *= n - i + 1; result /= i; } return result; }
int Ceil(int a, int b){return (a + b - 1) / b;}
int pow(int a, int b){ int res = 1; while (b){ if(b&1){ res *= a; b--;} a *= a; b /= 2; } return res;}
//_________________template______________

template<typename T> istream& operator >> (istream &istream, vector<T> &v) {for (auto &it : v) cin >> it; return istream;}
template <typename T> ostream& operator<<(ostream &os, const vector<T> &v) {for (auto e : v){os << e << " ";}return os;}
vector<vector<int>>vis;

void sol()
{
    int n,m;cin>>n>>m;
    vis.resize(n);
    fill(all(vis), vector<int>(m));

    vector<vector<int>>v(n);
    fill(all(v), vector<int>(m));

    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            cin>>v[i][j];
        }
    }
    
    int cnt = 0;
    stack<pair<int,int>>st;

    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            if(!vis[i][j]){
                cnt++;
                int val = v[i][j];
                st.push({i,j});
                vis[i][j] = 1;
                while (st.size())
                {
                    int x = st.top().ff;
                    int y = st.top().ss;
                    st.pop();   
                    if(x-1 >= 0 && !vis[x-1][y] && v[x-1][y] == val){
                        st.push({x-1, y});
                        vis[x-1][y] = 1;
                    }
                    if(x+1 < n && !vis[x+1][y] && v[x+1][y] == val){
                        st.push({x+1, y});
                         vis[x+1][y] = 1;
                    }
                    if(y-1 >= 0 && !vis[x][y-1] && v[x][y-1] == val){
                        st.push({x, y-1});
                        vis[x][y-1] = 1;
                    }
                    if(y+1 <= m && !vis[x][y+1] && v[x][y+1] == val){
                        st.push({x, y+1});
                        vis[x][y+1] = 1;
                    }
                }
            }
        }
    }
    for(int i = 0; i<n; i++){
        int one =0, zero = 0;
        for(int j = 0; j<m; j++){
            if(v[i][j] == 0) zero++;
            else one++;
        }
        int temp = (one*(one-1))/2 + (zero*(zero-1))/2;
        cnt += temp;
    }
    for(int i = 0; i<m; i++){
        int one = 0, zero = 0;
        for(int j = 0; j<n; j++){
            if(v[j][i] == 0) zero++;
            else one++;
        }
        int temp = (one*(one-1))/2 + (zero*(zero-1))/2;
        cnt += temp;
    }
    cout << cnt << endl;
}
//Before Submit handle the case for 0 and 1
int32_t main()
{
    FastIO;
    //TxtIO;
    int tt;
    tt = 1;
    // cin >> tt;
    while (tt--)
    {
        sol();
    }
}